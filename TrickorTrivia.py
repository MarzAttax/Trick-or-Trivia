...
try:    # try ensures that GPIO will be cleared properly even if the app crashes
	...
	class App(Frame):
		...
		def quizMe(self, question, choice1, choice2, choice3, choice4, correctChoice, difficulty):
			...
			# choice1
			self.button_1 = Button(root, text=choice1, font=("Helvetica", fontSizeB), command= lambda: result[1](difficulty)) # lambda allows you to pass an argument 
			self.button_1.grid(row=4, column=1, pady=5, padx=(100, 10))

			# choice2
			self.button_2 = Button(root, text=choice2, font=("Helvetica", fontSizeB), command= lambda: result[2](difficulty))
			self.button_2.grid(row=4, column=3, sticky=W, padx=(100, 10))

			# choice3
			self.button_3 = Button(root, text=choice3, font=("Helvetica", fontSizeB), command= lambda: result[3](difficulty))
			self.button_3.grid(row=5, column=1, pady=5, padx=(100, 10))

			# choice4
			self.button_4 = Button(root, text=choice4, font=("Helvetica", fontSizeB), command= lambda: result[4](difficulty))
		...
		def wrong(self, difficulty):
			...
			self.label_7 = Label(root, text="Wrong, but thanks for playing.", font=("Helvetica", 32), bg="black", fg="green")
			self.label_7.grid(columnspan=6, pady=5, padx=(100, 10))
